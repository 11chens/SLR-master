...
from modules.actor_critic_slr import ActorCriticSLR
    ...
    def update(self):
        ...
        for obs_batch, next_obs_batch, critic_obs_batch, actions_batch, target_values_batch, advantages_batch, returns_batch, old_actions_log_prob_batch, \
            old_mu_batch, old_sigma_batch, hid_states_batch, masks_batch in generator:
                ...
                # --------- update: triplet loss ---------
                obs_hist, prop = self.actor_critic.extract(obs_batch)
                _, z = self.actor_critic.encode(obs_hist, prop)
                pred_next_z = self.actor_critic.trans(
                    torch.cat([z, actions_batch], dim=1))
                next_obs_hist, next_prop = self.actor_critic.extract(next_obs_batch) 
                _, targ_next_z = self.actor_critic.encode(next_obs_hist, next_prop)
                batch_size = z.size(0)
                ...
                loss = surrogate_loss + self.value_loss_coef * value_loss + entropy_loss + self.triplet_loss_coef*triplet_loss
        ...
        return mean_triplet_loss, mean_value_loss,mean_surrogate_loss
